...
public class LocalSongAdapter extends Adapter<ViewHolder> implements Observer {
	...
	private void reshViewHolder(final int position,
		...
		itemViewHolder.getunLikePopdownItemRelativeLayout().setOnClickListener(
				new OnClickListener() {

					@Override
					public void onClick(View arg0) {
						songInfo.setIslike(SongInfo.LIKE);
						itemViewHolder.getLikePopdownItemRelativeLayout()
								.setVisibility(View.VISIBLE);
						itemViewHolder.getunLikePopdownItemRelativeLayout()
								.setVisibility(View.GONE);

						ToastUtil.showCenterTextToast(context, "添加喜欢");

						SongDB.getSongInfoDB(context).updatLikeSong(
								songInfo.getSid(), SongInfo.LIKE);
						// print();
		...
		itemViewHolder.getLikePopdownItemRelativeLayout().setOnClickListener(
				new OnClickListener() {
					...
					public void onClick(View arg0) {
						...
						SongDB.getSongInfoDB(context).updatLikeSong(
								songInfo.getSid(), SongInfo.UNLIKE);
		...
		if (songInfo.getIslike() == SongInfo.LIKE) {
			itemViewHolder.getLikePopdownItemRelativeLayout().setVisibility(
					View.VISIBLE);
			itemViewHolder.getunLikePopdownItemRelativeLayout().setVisibility(
					View.GONE);
		} else {
			itemViewHolder.getLikePopdownItemRelativeLayout().setVisibility(
					View.GONE);
	...
	protected void deleteSong(SongInfo songInfo, int position) {
		...
		SongMessage songMessage = new SongMessage();
		songMessage.setSongInfo(songInfo);
		songMessage.setType(SongMessage.LOCALDELMUSIC);
	...
	class ItemViewHolder extends ViewHolder {
		...
		private PopdownItemRelativeLayout unLikePopdownItemRelativeLayout;
		...
		public PopdownItemRelativeLayout getunLikePopdownItemRelativeLayout() {
			if (unLikePopdownItemRelativeLayout == null) {
				unLikePopdownItemRelativeLayout = (PopdownItemRelativeLayout) itemView
						.findViewById(R.id.unlike);
			}
			return unLikePopdownItemRelativeLayout;
		}
